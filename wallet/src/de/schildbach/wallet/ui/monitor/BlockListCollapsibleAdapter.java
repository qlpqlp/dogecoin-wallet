...
/**
 * Collapsible adapter for block list with modern UI
 * 
 * @author AI Assistant
 */
public class BlockListCollapsibleAdapter extends ListAdapter<BlockListCollapsibleAdapter.ListItem, RecyclerView.ViewHolder> {
    public static List<ListItem> buildListItems(final Context context, final List<StoredBlock> blocks, final Date currentTime,
        ...
        for (final StoredBlock block : blocks) {
            ...
            final List<ListItem.TxItem> transactionItems = buildTransactionItems(context, blockHash, transactions,
                    wallet, addressBook);
            items.add(new ListItem.BlockItem(blockHash, height, time, format, transactionItems, block));
        }
        return items;
    }
    ...
    private static List<ListItem.TxItem> buildTransactionItems(final Context context, final Sha256Hash blockHash,
        ...
        android.util.Log.d("BlockListCollapsibleAdapter", "buildTransactionItems called for block " + blockHash + 
                          " with " + (transactions != null ? transactions.size() : 0) + " transactions, wallet: " + (wallet != null ? "present" : "null"));
    ...
    public static abstract class ListItem {
        ...
        public static class BlockItem extends ListItem {
            ...
            public final StoredBlock block;

            public BlockItem(final Sha256Hash blockHash, final int height, final String time,
                             final MonetaryFormat format, final @Nullable List<TxItem> transactions, final StoredBlock block) {
                super(id(blockHash));
    ...
    public BlockListCollapsibleAdapter(final Context context, @Nullable final OnClickListener onClickListener,
    ...
    @Override
    public void submitList(@Nullable List<ListItem> list) {
        // Initialize expansion states for new list
        if (list != null) {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(final ViewGroup parent, final int viewType) {
        if (viewType == VIEW_TYPE_BLOCK)
            return new BlockViewHolder(inflater.inflate(R.layout.block_row_collapsible, parent, false));
    ...
    public void onBindViewHolder(final RecyclerView.ViewHolder holder, final int position,
        ...
        if (holder instanceof BlockViewHolder) {
            ...
            final boolean isExpanded = position < expandedStates.size() && expandedStates.get(position);
            ...
            if (fullBind) {
                ...
                // Set expanded view data (simplified - only showing hash)
            ...
            if (isExpanded) {
                blockHolder.expandedView.setVisibility(View.VISIBLE);
                blockHolder.expandIcon.setImageResource(R.drawable.ic_expand_less);
            } else {
                blockHolder.expandedView.setVisibility(View.GONE);
                blockHolder.expandIcon.setImageResource(R.drawable.ic_expand_more);
            }
    ...
    public class BlockViewHolder extends RecyclerView.ViewHolder {
        ...
        private BlockViewHolder(final View itemView) {
            ...
            // Collapsible UI elements
            collapsedView = itemView.findViewById(R.id.block_collapsed_view);
            expandedView = itemView.findViewById(R.id.block_expanded_view);
            expandIcon = itemView.findViewById(R.id.block_expand_icon);
            ...
            itemView.setOnClickListener(v -> {
                ...
                if (position != RecyclerView.NO_POSITION) {
                    ...
                    BlockListCollapsibleAdapter.this.toggleExpansion(position);
                }
            });
        }
    }
